...
# Reduce to cores with rings in the last 500 years and 30 year series
if(FALSE) {
cores <- raw %>%
  group_by(core) %>%
...
log_y <- log(y_orig + 0.00001) # add tiny increase for years with zero growth (missing rings?)
...
plot(m_negexp_norm[ ,c("sd_eta", "sd_x", "beta0")])
plot(m_negexp_norm[ ,c("sd_eta[1]", "sd_x", "beta0[1]")])
plot(m_negexp_norm[ ,c("sd_eta[1]", "sd_eta[2]", "sd_eta[3]", "sd_x", "beta0[1]", "beta0[2]", "beta0[3]")])

plot(m_negexp_norm[ ,c("sd_eta", "sd_x", "beta0[1]", "beta0[2]", "beta0[3]")])
par(mfrow = c(1,1))
...
#### climate spline 50 year knots - not getting reasonable fit w/25 or 50 year knots #####
...
cl <- makeCluster(nc)                       # Request # cores
clusterExport(cl, c("m2_nc_data", "initialize_m2_nc", "params", "M", "f", "l", "K", "Tea", "a_use", "log_y", "x_use", "nb", "ni", "nt"))
clusterSetRNGStream(cl = cl, 867530123)
system.time({ 
  out <- clusterEvalQ(cl, {
    library(rjags)
    jm <- jags.model("Code/JAGS/m2_nc_multisp.txt", m2_nc_data, initialize_m2_nc, n.adapt=nb, n.chains=1) # Compile model and run burnin
